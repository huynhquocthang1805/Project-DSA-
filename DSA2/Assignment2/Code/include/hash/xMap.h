...
V xMap<K, V>::put(K key, V value)
{
  ...
  // Tính toán chỉ số băm cho khóa dựa trên capacity
  int index = this->hashCode(key, capacity);
  V retValue = value;

  // Truy xuất danh sách liên kết tại chỉ số băm
  ...
  for (auto entry : temp)
  {
    ...
    {
      // Nếu tìm thấy khóa, cập nhật giá trị mới và trả về giá trị cũ
  ...
  // Nếu không tìm thấy khóa, tạo một đối tượng Entry mới và thêm vào cuối danh sách
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K))
{
  ...
  for (auto start = temp.begin(); start != temp.end(); start++)
  {
    if (keyEQ(key, (*start)->key) != 0)
    {
      V oldValue = (*start)->value;
      if (deleteKeyInMap != nullptr)
        deleteKeyInMap((*start)->key);

      temp.removeItem((*start), xMap<K, V>::deleteEntry);
      this->count--;
      return oldValue;
    }
  }
                        ...
                        void (*deleteValueInMap)(V))
{
  ...
  for (auto start : list)
  {
    if (keyEQ(key, start->key) != 0 && valueEQ(value, start->value) != 0)
    {
      if (deleteKeyInMap != 0)
        deleteKeyInMap(start->key);
      if (deleteValueInMap != 0)
        deleteValueInMap(start->value);

      list.removeItem(start, xMap<K, V>::deleteEntry);
      this->count--;
      return true;
    }
  }
...
bool xMap<K, V>::containsKey(K key)
{
  ...
  // Nếu tìm thấy (pos khác -1), trả về true, ngược lại trả về false
  return false;
}
...
{
  // TODO YOUR CODE IS HERE
  // Duyệt qua tất cả các danh sách liên kết trong bảng băm
  for (int i = 0; i < capacity; ++i)
  {
    // Lấy danh sách liên kết tại vị trí i
    DLinkedList<Entry *> &temp = this->table[i];

    // Tìm kiếm giá trị trong danh sách liên kết
    for (auto start = temp.begin(); start != temp.end(); start++)
    {
      if (valueEQ(value, (*start)->value) != 0)
      {
        return true;
      }
    }
...
{
  // TODO YOUR CODE IS HERE
  DLinkedList<K> keyList; // Tạo danh sách để chứa các khóa

  // Duyệt qua tất cả các chỉ số băm trong bảng băm
  for (int i = 0; i < capacity; ++i)
  {
    // Lấy danh sách liên kết tại vị trí i
    DLinkedList<Entry *> &temp = this->table[i];
  ...
  return keyList; // Trả về danh sách các khóa
}
...
DLinkedList<V> xMap<K, V>::values()
{
  ...
  return valueList; // Trả về danh sách các giá trị
}
...
{
  // TODO YOUR CODE IS HERE
  DLinkedList<int> clashCounts; // Tạo danh sách để chứa số phần tử tại từng địa chỉ
  ...
  for (int i = 0; i < capacity; ++i)
  {
    ...
    // Đếm số phần tử trong danh sách liên kết và thêm vào danh sách clashCounts
    int count = temp.getCount();
    clashCounts.add(count); // Thêm số phần tử vào danh sách kết quả
  }
...
void xMap<K, V>::removeInternalData()
{
  ...
  for (int idx = 0; idx < this->capacity; idx++)
  {
    ...
    for (auto pEntry : list)
    {
      delete pEntry; // Delete each entry (key-value pair)
    }
    list.clear(); // Clear the linked list at this index
  }
